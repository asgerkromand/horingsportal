...
class CustomPDF(FPDF):
    def __init__(self):
        ...
        self.add_font('DejaVuSansCondensed', '', str(path_to_external_fonts / "DejaVuSansCondensed.ttf"))
        self.add_font('DejaVuSansCondensed', 'B', str(path_to_external_fonts / "DejaVuSansCondensed-Bold.ttf"))
        self.add_font('DejaVuSansCondensed', 'I', str(path_to_external_fonts / "DejaVuSansCondensed-Oblique.ttf"))

    def header(self):
        self.set_font("DejaVuSansCondensed", "B", 12)
        self.cell(0, 10, "Email Document", 0, new_x=XPos.LMARGIN, new_y=YPos.NEXT, align='C')
        self.ln(3)
...
def convert_email_file(input_file: Path, output_dir: Path, output_format: str = "pdf") -> None:
    ...
    email_data, attachments = parse_eml_file(eml_file, output_dir)
    if not email_data:
        return
...
def parse_eml_file(eml_file: Path, output_dir: Path) -> Optional[Dict[str, Union[str, None]]]:
    ...
    try:
        ...
        email_data = {
            ...
            "body_text": (msg.get_body(preferencelist=("plain")).get_content() if msg.get_body(preferencelist=("plain")) else "(No Body Content)"),
        }

        # Handle attachments only if filename contains "ringssvar" or "ringsvar"
        attachments = [save_attachment(part, output_dir) for part in msg.iter_attachments() 
                       if part.get_filename() and ("ringssvar" in part.get_filename().lower() or "ringsvar" in part.get_filename().lower())]
        
        return email_data, [attach for attach in attachments if attach]  # Filter out None attachments
...
def create_pdf(email_data: Dict[str, str], attachments: list, output_dir: Path, file_stem: str) -> Path:
    pdf_file = output_dir / f"{file_stem}.pdf"
    pdf = CustomPDF()
    pdf.set_auto_page_break(auto=True, margin=10)

    # Add a new page only if there is body text to display
    if email_data.get("body_text").strip():  # Check if body_text is not empty or just whitespace
        pdf.add_page()
        ...
        for key in ["subject", "sender", "recipients", "date", "body_text"]:
            ...
            pdf.cell(cell_width, 10, f"{key.replace('_', ' ').capitalize()}:", 0, new_x=XPos.LMARGIN, new_y=YPos.NEXT)
...
def merge_pdfs(main_pdf_path: Path, attachment_pdfs: list, output_dir: Path, file_stem: str) -> Path:
...
def process_attachment(attachment_path: Path, output_dir: Path, output_format: str) -> Optional[Path]:
    ...
    try:
        if ext in ['.doc', '.docx', '.rtf', '.txt', '.htm', '.html', '.mht', '.xlsx', '.xls']:
            ...
            if output_format in ['png', 'jpg']:
                convert_pdf_to_image(output_file.with_suffix('.pdf'), output_dir, original_file_name, output_format)
            ...
            if output_format == 'pdf':
                ...
                convert_pdf_to_image(attachment_path, output_dir, original_file_name, output_format)
...
def convert_to_pdf(input_file: Path, output_dir: Path) -> None:
    ...
    subprocess.run(["soffice", "--headless", "--convert-to", "pdf", "--outdir", str(output_dir), str(input_file)], check=True)

def process_image_attachment(attachment_path: Path, output_dir: Path, output_format: str) -> None:
    """Handles the processing of image attachments (e.g., TIFF, JPG)."""
    images = convert_from_path(str(attachment_path)) if attachment_path.suffix.lower() in ['.tif', '.tiff'] else [Image.open(attachment_path)]
    if output_format == 'pdf':
        images[0].save(output_dir / f"{attachment_path.stem}.pdf", save_all=True, append_images=images[1:])
    else:
        for i, img in enumerate(images):
            img.save(output_dir / f"{attachment_path.stem}_{i}.{output_format}", output_format.upper())

def convert_pdf_to_image(pdf_file: Path, output_dir: Path, file_stem: str, output_format: str) -> None:
    ...
    try:
        ...
        for i, img in enumerate(images):
            ...
            img.save(img_output, 'JPEG' if output_format == 'jpg' else output_format.upper())
...
def process_input(input_path: str, output_base_dir: str, output_format: str) -> None:
    ...
    if input_path.is_dir():
        process_folder(input_path, output_base_dir, output_format)
    elif input_path.is_file():
        process_file(input_path, output_base_dir, output_format)
    else:
        logging.error("Invalid input path. Please provide a valid file or folder.")

def process_folder(input_folder: Path, output_base_dir: Path, output_format: str) -> None:
    ...
    for file in files:
        ...
        convert_email_file(file, output_dir, output_format)

def process_file(input_file: Path, output_base_dir: Path, output_format: str) -> None:
    """Processes a single .msg or .oft file."""
    if input_file.suffix.lower() not in [".msg", ".oft"]:
        logging.error("Unsupported file format. Please provide a .msg or .oft file.")
        return

    case_name = input_file.parent.name  # Use the parent folder name as the case name
    output_dir = output_base_dir / case_name / input_file.stem
    output_dir.mkdir(parents=True, exist_ok=True)

    convert_email_file(input_file, output_dir, output_format)
